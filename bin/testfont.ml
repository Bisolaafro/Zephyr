...
let () =
  ...
  let window =
    Window.create ~pos:(`centered, `centered) ~dims:(width, height)
      ~title:"SDL2 Sprite Surface" ~flags:[ Window.Resizable ]
  in
  ...
  let render t =
    Surface.fill_rect ~dst:screen ~rect:screen_rect ~color:0xFFFF00l;
    let y = t / 10 mod width in
    let x = t / 10 mod width in
    let dst_rect = { dst_rect with Rect.y } in
    let dst_rect = { dst_rect with Rect.x } in
    let _ = Surface.blit_surface ~src:surf ~dst:screen ~src_rect ~dst_rect in
